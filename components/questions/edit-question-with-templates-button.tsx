    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <div className="space-y-4">
            <div>
              <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">Atribuir aos Cadernos</h3>
            ...
            {allTemplates && allTemplates.length > 0 ? (
              <div className="space-y-2 max-h-[200px] overflow-y-auto border border-border/50 rounded-lg p-3">
                {allTemplates.map((template) => (
                  <label
                    key={template.id}
                    htmlFor={`template-${template.id}`}
                    className="flex items-center gap-3 p-3 rounded-lg border-2 border-border/50 hover:border-primary/30 hover:bg-accent/50 cursor-pointer transition-all"
                  >
                    <Checkbox
